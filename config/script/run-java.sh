...
init_limit_env_vars() {
  # Read in container limits and export the as .env variables
...
run() {
  ...
  # === 여기서 JAR 파일 경로 로그 찍기 ===
    echo "[run-java.sh] JAVA_APP_JAR=${JAVA_APP_JAR:-<unset>}"
    echo "[run-java.sh] JAVA_MAIN_CLASS=${JAVA_MAIN_CLASS:-<unset>}"
    echo "[run-java.sh] 실행 명령: java $(java_options) -cp \"$(classpath)\" ${args} $@"
